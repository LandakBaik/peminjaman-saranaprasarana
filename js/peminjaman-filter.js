document.addEventListener('DOMContentLoaded', function () {
    ...
    const filterPeminjam =
        document.getElementById('filterPeminjam');

    const filterStatusKembali =
        document.getElementById('filterStatusKembali');
    ...
    if (btnReset) {
        ...
        btnReset.addEventListener('click', function () {
            ...
            if (filterStatusKembali) {
                filterStatusKembali.value = '';
            }
    ...
    function applyAll() {
        ...
        const roomVal =
            filterRoom
            ? filterRoom.value.toLowerCase().trim()
            : '';

        const peminjamVal =
            filterPeminjam
            ? filterPeminjam.value.toLowerCase().trim()
            : '';

        const statusKembaliVal =
            filterStatusKembali
            ? filterStatusKembali.value
            : '';
        ...
        rows.forEach(row => {
            ...
            const peminjam =
                (row.getAttribute('data-peminjam') || '')
                .toLowerCase();

            const statusKembali =
                row.getAttribute('data-status-kembali');
            ...
            const matchRoom =
                !roomVal ||
                room.includes(roomVal);

            const matchPeminjam =
                !peminjamVal ||
                peminjam.includes(peminjamVal);

            const matchStatusKembali =
                !statusKembaliVal ||
                statusKembali === statusKembaliVal;
